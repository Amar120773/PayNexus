...
import { NetworkScoreResult, ScoreResult } from "@/lib/api";
import { useRouter } from "next/navigation";
import { ShieldAlert, ArrowRight, CornerUpLeft, Network } from "lucide-react";
...
function NetworkGraphCore({ centralMerchantId, centralNodeData, networkData, loading }: NetworkGraphProps) {
  ...
  const { nodes, edges, highRiskCount } = useMemo(() => {
    ...
    neighbors.forEach((neighbor, i) => {
      ...
      // Simple unlabelled deterministic edge (API does not provide shared IP / Device labels)
      newEdges.push({
  ...
  }, [networkData, centralMerchantId, centralNodeData]);

  const onNodeClick = useCallback((_: any, node: Node) => {
    const res = networkData?.results.find(r => r.merchant_id === node.id);
    if (res && res.merchant_id !== centralMerchantId) {
      setSelectedNode(res);
    } else {
      setSelectedNode(null);
    }
  }, [networkData, centralMerchantId]);
  ...
  if (!networkData || networkData.results.length <= 1) {
    return (
      <div style={{ padding: 64, textAlign: "center", border: "1px dashed var(--border-default)", background: "var(--bg-subtle)", borderRadius: "var(--radius-lg)" }}>
        ...
        <h3 className="font-display" style={{ fontSize: 20, fontWeight: 500, color: "var(--text-primary)", marginBottom: 8 }}>
  ...
  return (
    <div className="animate-fade-in" style={{ display: "flex", flexDirection: "column", gap: 32 }}>
      ...
      <div style={{ display: "flex", gap: 32, flexWrap: "wrap", padding: 24, border: "1px solid var(--border-default)", background: "var(--bg-subtle)", borderRadius: "var(--radius-md)" }}>
        <div>
          <div className="font-data" style={{ fontSize: 10, color: "var(--text-muted)", letterSpacing: "0.1em", marginBottom: 4 }}>DIRECT CONNECTIONS</div>
          <div className="font-data" style={{ fontSize: 24, fontWeight: 700, color: "var(--text-primary)" }}>{networkData.results.length - 1}</div>
        </div>
        <div>
        ...
        <div>
          <div className="font-data" style={{ fontSize: 10, color: "var(--text-muted)", letterSpacing: "0.1em", marginBottom: 4 }}>RELATIONSHIP TYPES</div>
          <div className="font-data" style={{ fontSize: 24, fontWeight: 700, color: "var(--text-secondary)" }}>IMPLICIT API EDGES</div>
        </div>
      </div>
